  ...
  for (const meetup of meetups ?? []) {
    ...
    if (successor) {
      const { error: transferError } = await admin
        .from('meetups')
        .update({ created_by: (successor as ParticipantRow).user_id })
        .eq('id', meetup.id);
...
/**
 * Lista paths en meetup-covers donde el userId aparece como segundo segmento
 * del path ({meetupId}/{userId}/{timestamp}.jpg).
 */
const listUserCoverPaths = async (
  ...
  const { data, error } = await admin
    ...
    .from('objects')
    .select('name')
    .eq('bucket_id', COVERS_BUCKET)
    .like('name', `%/${userId}/%`);

  if (error) {
    console.warn('No se pudieron listar portadas en Storage:', error.message);
    return [];
  }

  return (data ?? [])
    .map((row) => (row as StorageObjectRow).name)
    .filter((name) => name.split('/')[1] === userId);
};
