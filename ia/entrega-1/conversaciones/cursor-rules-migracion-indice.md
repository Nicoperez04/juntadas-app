...
Listo. Creé la carpeta `.cursor/rules/` en la raíz del repo y el archivo de reglas del proyecto.

**Archivo creado:** `.cursor/rules/project.mdc`

Agregué frontmatter YAML mínimo (`alwaysApply: true`) para que Cursor aplique estas reglas en todas las conversaciones del proyecto. El contenido es el que indicaste, con el título `# Bitácora de Juntadas — Cursor Rules` (interpreté `markdown#` como el encabezado markdown).

Si preferís que la regla aplique solo a ciertos archivos (por ejemplo `juntadas-app/mobile/**`) en lugar de siempre, decime y lo ajusto.

---

### Archivos modificados

- `.cursor/rules/project.mdc` — editado
...
Listo. Creé el archivo con la migración completa en:

**`juntadas-app/supabase/migrations/001_initial_schema.sql`**

Lo ubiqué dentro de `juntadas-app/supabase/`, que ya existía con su `README.md`. Incluye todo el SQL que pasaste: enums, tablas (`profiles`, `meetups`, `meetup_participants`, `impostor_games`, `memories`), índices, triggers (`handle_new_user`, `update_updated_at`) y policies de RLS.

---

### Archivos modificados

- `juntadas-app/supabase/migrations/001_initial_schema.sql` — editado
...
Actualicé el índice de IA con el contenido que indicaste.

**Archivo modificado:** `juntadas-app/ia/entrega-1/indice_ia.md`

Incluye las 12 entradas de Claude y las 3 de Cursor, organizadas por sección.
