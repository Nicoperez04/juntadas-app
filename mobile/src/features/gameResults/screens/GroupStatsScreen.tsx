...
import { useGroupGameResults } from '../hooks/useGroupGameResults';

type NavProp = NativeStackNavigationProp<MainStackParamList, 'GroupStats'>;
type RouteProps = RouteProp<MainStackParamList, 'GroupStats'>;

export const GroupStatsScreen = () => {
  const navigation = useNavigation<NavProp>();
  const route = useRoute<RouteProps>();
  const { groupId, groupName } = route.params;

  const { stats, isLoading, error, refresh } = useGroupGameResults(groupId);
      ...
      {isLoading && !stats ? (
        ...
        <View style={styles.centerState}>
          ...
          <Text style={styles.stateText}>
            Los resultados apareceran cuando se guarden partidas desde juntadas de
            este grupo.
          </Text>
        </View>
      ) : (
          ...
          <View style={styles.section}>
            ...
            {recentResults.map((result) => (
              <GameResultCard
                key={result.id}
                result={result}
                contextLabel={result.meetupTitle}
              />
            ))}
            {hasMoreResults ? (
              <AppButton
                ...
                variant="ghost"
                onPress={() =>
                  navigation.navigate(Routes.GroupResultsHistory, {
                    groupId,
                    groupName,
                  })
                }
