...
type NavProp = NativeStackNavigationProp<MainStackParamList, 'TournamentSetup'>;
type RouteProps = RouteProp<MainStackParamList, 'TournamentSetup'>;

export const TournamentSetupScreen = () => {
  ...
  const handleAddPlayer = () => {
    ...
    if (players.some((p) => p.toLowerCase() === trimmed.toLowerCase())) {
      setErrorMessage('Este participante ya está en la lista');
      setShowError(true);
      return;
    }
  ...
  const handleStart = () => {
    if (players.length < 4) {
      setErrorMessage('Debes agregar al menos 4 participantes para iniciar el torneo');
      setShowError(true);
      return;
    }

    void triggerSelectionHaptic();
    navigation.navigate(Routes.TournamentGame, {
      players,
      meetupId,
    });
  };
    ...
    <SafeAreaView style={styles.root} edges={['top', 'bottom']}>
      ...
      >
        <View style={styles.header}>
          ...
          <Text style={styles.headerTitle}>Nuevo Torneo Playoffs</Text>
        </View>

        <ScrollView contentContainerStyle={styles.scrollContent} keyboardShouldPersistTaps="handled">
          <View style={styles.card}>
            <Text style={styles.label}>Nuevo Participante</Text>
            ...
            <Text style={[styles.label, { marginTop: theme.spacing.md }]}>Participantes</Text>
            {players.length === 0 ? (
              <Text style={styles.emptyText}>No hay participantes agregados.</Text>
            ) : (
              <View style={styles.list}>
                {players.map((name, idx) => (
                  <View key={`team-${idx}`} style={styles.playerRow}>
        ...
        <View style={styles.footer}>
          <AppButton label="Armar Playoff" onPress={handleStart} />
        </View>
      </KeyboardAvoidingView>
